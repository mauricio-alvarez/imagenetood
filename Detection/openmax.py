# derived from https://github.com/ma-xu/Open-Set-Recognition/blob/master/OSR/OpenMax/openmax.py
import numpy as np
...
def compute_train_score_and_mavs_and_dists(train_logits, train_y, num_classes):
    scores = []
    subset = np.argmax(train_logits,1) == train_y
    for i in range(num_classes):
        scores.append(np.expand_dims(train_logits[np.bitwise_and(subset, train_y == i),:], 1))
    mavs = np.array([np.mean(x, axis=0) for x in scores])  # (C, 1, C)
    dists = []
    for mcv, score in tqdm(zip(mavs, scores)):
        dists.append(compute_channel_distances(mcv, score))
...
def calc_openmax(weibull_model,
            categories,
            input_score,
            eu_weight,
            alpha=10,
...
def openmax(eval_logits, num_classes, mavs, dists, weibull_tail=20, weibull_alpha = 3):
    #_, mavs, dists = compute_train_score_and_mavs_and_dists(train_logits, train_y, num_classes)
    #with open("openmax.pkl", "wb") as f:
    #    pickle.dump({'mavs': mavs, 'dists': dists}, f, protocol=pickle.HIGHEST_PROTOCOL)
    categories = list(range(num_classes))
    model = fit_weibull(mavs, dists, categories, weibull_tail, 'euclidean')
    scores = []
    for i in range(eval_logits.shape[0]):
        ss = calc_openmax(model, categories, eval_logits[i:(i+1)], 0.5, weibull_alpha, 'euclidean')
        scores.append(np.max(ss))
